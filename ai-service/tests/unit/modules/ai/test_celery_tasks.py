...
from app.infrastructure.tasks.celery_tasks import generate_embeddings_task, summarize_conversation_task
...
def test_generate_embeddings_task(mock_factory_cls):
    ...
    mock_response = EmbeddingResponse(
        model="gemini-embedding-001", provider="gemini", embeddings=[[0.1, 0.2], [0.3, 0.4]], usage=UsageDTO()
    )
