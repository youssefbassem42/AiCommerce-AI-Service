...
class TestCategoryService:
    ...
    async def test_get_category_found(self, service, mock_repo):
        mock_repo.find_by_id.return_value = Category(id="c1", store_id="s1", organization_id="o1", name="Test")
    ...
    async def test_list_categories(self, service, mock_repo):
        mock_repo.paginate.return_value = (
            [Category(id="c1", store_id="s1", organization_id="o1", name="C1")],
            1,
        )
    ...
    async def test_update_category(self, service, mock_repo):
        existing = Category(id="c1", store_id="s1", organization_id="o1", name="Old")
        mock_repo.find_by_id.return_value = existing
        mock_repo.update.return_value = Category(id="c1", store_id="s1", organization_id="o1", name="Updated")
    ...
    async def test_get_children(self, service, mock_repo):
        mock_repo.find_children.return_value = [
            Category(id="c2", store_id="s1", organization_id="o1", name="Child", parent_id="c1")
        ]
    ...
    async def test_get_root_categories(self, service, mock_repo):
        mock_repo.find_root.return_value = [Category(id="c1", store_id="s1", organization_id="o1", name="Root")]
