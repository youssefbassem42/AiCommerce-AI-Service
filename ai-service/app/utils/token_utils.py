...
def calculate_tokens(text: str, model_name: str = "gpt-4o") -> int:
    ...
    if tiktoken and provider in ["openai", "azure", "deepseek"]:
        try:
            ...
            if "gpt-4" in model_name or "gpt-3.5" in model_name or "o1" in model_name:
                encoding_name = "cl100k_base"
...
def estimate_price(text: str, is_completion: bool, model_name: str) -> float:
    ...
    cost_per_1m = model_info.pricing.completion_cost_per_1m if is_completion else model_info.pricing.prompt_cost_per_1m
