...
from app.infrastructure.mongodb.collections import get_bundle_suggestions_collection, get_recommendations_collection
...
class RecommendationRepository(BaseMongoRepository[RecommendationDocument, Recommendation], IRecommendationRepository):
    ...
    async def get_active_bundles(self, store_id: str, session: Any = None) -> list[BundleSuggestion]:
        ...
        try:
            cursor = self.bundles_collection.find({"store_id": store_id, "status": "active"}, session=session).sort(
                "created_at", -1
            )
