    ...
    async def chat(self, request: ChatRequest, timeout: float | None = None) -> ChatResponse:
        async def _run():
            ...
            async with httpx.AsyncClient() as client:
                response = await client.post(f"{self.base_url}/api/chat", json=payload, timeout=actual_timeout)
    ...
    async def stream(
        ...
        try:
            async with (
                ...
                client.stream("POST", f"{self.base_url}/api/chat", json=payload, timeout=actual_timeout) as response,
            ):
                response.raise_for_status()
            ...
            async with httpx.AsyncClient() as client:
                for text in inputs:
                    ...
                    response = await client.post(
                        f"{self.base_url}/api/embeddings", json=payload, timeout=actual_timeout
                    )
                    response.raise_for_status()
                    data = response.json()
