...
def summarize_conversation_task(conversation_id: str, model: str = "gpt-4o-mini") -> str:
    ...
    async def _async_run():
        ...
        request = ChatRequest(messages=[MessageDTO(role="user", content=prompt)], model=model)
        ...
        await repo.collection.update_one({"conversation_id": conversation_id}, {"$set": {"metadata": metadata}})
        return summary
